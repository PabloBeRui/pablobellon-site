...
`,f='<button id="lang-btn" class="lang-toggle-btn nav-button"><img src="../../assets/images/icons/globe_green_transparent_language.png" id="lang-ico"> EN</button>',w='<button id="lang-btn" class="lang-toggle-btn nav-button"><img src="../../assets/images/icons/globe_green_transparent_language.png" id="lang-ico"> ES</button>',m="Junior Developer with a relentless growth mindset. 100% driven, always learning 👾 ",k=t=>{document.getElementById("typewriter-text").innerHTML="";const e=m,a=document.getElementById(t);let o=0;function s(){if(o<e.length)a.innerHTML+=e.charAt(o),o++,setTimeout(s,60);else{a.classList.add("typewriter-class-2");const n=new CustomEvent("typewriterFinished");document.dispatchEvent(n)}}a&&s()},S=()=>{v("coming-soon-vid"),g("bg-video"),k("typewriter-text")},l=`  
...
`;t.insertAdjacentHTML("beforeend",n)}),document.querySelectorAll(".course-summary").forEach(e=>{e.addEventListener("click",()=>{e.parentElement.classList.toggle("expanded")})})};let p=!1;const E=()=>{const t=document.querySelector("#description-div"),e=document.querySelector("#lang-btn-div"),a=()=>{p?(t.innerHTML=h,e.innerHTML=f):(t.innerHTML=y,e.innerHTML=w),document.querySelector("#lang-btn").addEventListener("click",()=>{p=!p,a()})};a()},I=`<div id="about-me-div-content" class="smooth-fade-in-upward">
...
`,u=(t,e)=>{t.classList.add("smooth-fade-out-upward"),setTimeout(()=>{t.innerHTML=e,t.classList.remove("smooth-fade-out-upward")},1e3)},H=document.querySelectorAll("link[data-theme-link]");let c=sessionStorage.getItem("theme")||"80s";function b(t){H.forEach(e=>{e.href=e.href.replace(/\/(80s|present)\//,`/${t}/`)})}function B(){c=c==="80s"?"present":"80s",b(c),sessionStorage.setItem("theme",c)}b(c);let r=sessionStorage.getItem("theme")==="present";const A=()=>{r=!r,sessionStorage.setItem("theme",r?"present":"80s")},D=(t,e)=>{const a=document.querySelector("#nav-buttons"),o=document.querySelector("#change-style-button-nav");document.addEventListener("typewriterFinished",()=>{a.classList.add("visible"),o.classList.add("visible")});const s=document.querySelector(t);s.addEventListener("click",n=>{n.target?.id==="about-me-button"?(s.innerHTML=I,E()):n.target?.id==="my-progress-button"?(s.innerHTML=M,j()):n.target?.id==="back-button"?u(s,r?d:l):n.target.closest?.("#change-style-button")&&(setTimeout(()=>{B()},1e3),A(),u(s,r?d:l))})},C=t=>{window.addEventListener("click",e=>{const a=document.querySelector("#section-global-container"),o=e.target.closest(t),s=e.target.closest("button");!o&&!s&&u(a,r?d:l)})};document.addEventListener("DOMContentLoaded",()=>{L()||S(),D("#section-global-container"),C("#section-global-container")});
